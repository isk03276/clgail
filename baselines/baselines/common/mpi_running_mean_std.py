...
class RunningMeanStd(object):
    ...
    def __init__(self, epsilon=1e-2, shape=(), worker_id=-1):
        if worker_id == -1:
            ...
            self._sum = tf.get_variable(
                ...
                shape=shape,
                initializer=tf.constant_initializer(0.0),
                name="runningsum{}".format(worker_id), trainable=False)
            self._sumsq = tf.get_variable(
                dtype=tf.float64,
                shape=shape,
                initializer=tf.constant_initializer(epsilon),
                name="runningsumsq{}".format(worker_id), trainable=False)
            self._count = tf.get_variable(
                dtype=tf.float64,
                shape=(),
                initializer=tf.constant_initializer(epsilon),
                name="count{}".format(worker_id), trainable=False)
