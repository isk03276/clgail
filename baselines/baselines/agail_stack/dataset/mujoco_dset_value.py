...
class Dset(object):
    def __init__(self, inputs, labels, rets, randomize):
        self.inputs = inputs
        self.labels = labels
        self.rets = rets
...
class Mujoco_Dset(object):
    def __init__(self, expert_path, train_fraction=0.7, traj_limitation=-1, randomize=True):
        ...
        if len(obs.shape) > 2:
            ...
            self.rets = np.reshape(rets, [-1, np.prod(rets.shape[2:])])
        else:
        ...
        assert len(self.obs) == len(self.acs) == len(self.rets)
        self.num_traj = min(traj_limitation, len(traj_data['obs']))
        self.num_transition = len(self.obs)
        self.randomize = randomize
        self.dset = Dset(self.obs, self.acs, self.rets, self.randomize)
