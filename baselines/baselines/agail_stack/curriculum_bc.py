...
class SubGoalGenerator:
	...
	def build_model(self):
		...
		value_network.add(Dense(100, input_dim=self.state_size, activation='relu', kernel_initializer='he_uniform'))
		value_network.add(Dense(100, activation='relu', kernel_initializer='he_uniform'))
	...
	def train(self, total_states, learning_iteration=500, batch_size=1):
		"""
		...
		for iter in range(learning_iteration):
			for states in total_states:
				if len(states) <= batch_size:
					...
					rews[-1] = 1
					self.train_model(states.reshape(len(self.state_sizes), self.state_size), rews, False)
				else:
	...
	def create_sub_goals(self, states, num_of_goals):
		"""
		...
		sub_goal_list = []
		for state in states:
			state_value_list.append(self.value_network.predict(state.reshape(1, self.state_size))[0])
	...
	def predict_state_score(self, states):
		"""
		input : list of state 
		output : min score of states
		"""
		value = self.value_network.predict(states.reshape(len(states), self.state_size))[0]
