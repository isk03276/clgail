...
class Jaco2PickPlaceEnv():
    def __init__(self,
        ...
        model_path = "jaco_arm2_pickplace.xml"
    ...
    def step(self, a):
        ...
        distance_target1_ontarget2 = np.linalg.norm(self.sim.data.get_body_xpos("target1") - (self.sim.data.get_body_xpos("target2") + [0, 0, 0.001]))
        if hand_d < 0.1:
            reward += 0.1
            if self.sim.data.get_body_xpos("target1")[2] > 0.06 and self.sim.data.get_body_xpos("target1")[2] < 0.2:
                reward += 10 + self.sim.data.get_body_xpos("target1")[2] + min(1/abs(distance_target1_ontarget2), 1000)
